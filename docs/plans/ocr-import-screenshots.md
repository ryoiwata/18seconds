...
4. **Branch on `(answerVisible, explanationVisible)`**:
   - `(true, true)` — trust the screenshot, use its `correctAnswer` + `explanation` verbatim. `source: "ocr-visible"`.
   - `(true, false)` — trust the screenshot's `correctAnswer`, run a **single solve-only call** (no verify) to generate a 1–3 sentence explanation grounded in the known answer. `source: "ocr-visible-explanation-generated"`.
   - `(false, *)` and `--skip-solve` not set — run **solve** (Sonnet text), then **verify** (Sonnet, fresh context). Drop on disagreement. `source: "ocr-solved"`.
...
### 4a. Visible-answer + visible-explanation branch

`answerVisible === true && explanationVisible === true`. Use the screenshot's `correctAnswer` and `explanation` verbatim. No LLM follow-up. `source: "ocr-visible"`.

### 4b. Visible-answer + missing-explanation branch (explanation backfill)

`answerVisible === true && explanationVisible === false`. The screenshot's `correctAnswer` is treated as ground truth, but ingesting without any `explanation` would degrade post-session review (the user reads explanations after answering wrong; an empty one is worse than the marginal LLM cost). One Sonnet text-only call asks for a 1–3 sentence prose explanation **grounded in the given answer** — the LLM is told the correct answer up front and is not asked to second-guess it. No verify pass: there is nothing to disagree with. Returns:
...
`source: "ocr-visible-explanation-generated"`.
...
`source` is one of:
- `"ocr-visible"` — answer and explanation both lifted verbatim from the screenshot.
- `"ocr-visible-explanation-generated"` — answer lifted from the screenshot, explanation generated by a single solve-only Sonnet call grounded in the known answer.
- `"ocr-solved"` — neither was visible; answer + explanation came from solve+verify.
...
Successfully ingested: 243
  - visible answer (verbatim explanation):    130
  - visible answer (generated explanation):    50
  - solve+verify:                              63
```
...
Difficulty (anchored by question features, not by the latency thresholds the names suggest — those thresholds are for runtime mastery scoring, not OCR classification):
...
Estimate from question complexity. Ignore any "Difficulty: hard" label printed on the screenshot — those are unreliable across sources.
...
Used only in branch 4c (answer not visible).
...
### Explanation-backfill prompt (Sonnet 4.6, text only)

Used only in branch 4b (answer visible, explanation missing). The correct answer is given to the model — it should not second-guess it.

System:

```
You are writing a 1–3 sentence explanation for a CCAT (Criteria Cognitive Aptitude Test) multiple-choice question whose correct answer is already known. Do NOT second-guess the given answer. Describe the FASTEST path a test-taker would use to arrive at it. The explanation is shown during post-session review, so it is for sanity-checking and pattern reinforcement, not pedagogy. Keep it tight: 1–3 sentences, no preamble.

Respond with raw JSON only — no markdown code fences, no commentary, just the object:

{
  "explanation": <1–3 sentence string framing the fastest path to the given answer>
}
```
...
1. **Dry run, 5 from the answer-visible source:**
   ...
   ```
   Expected: 5 extractions printed to stdout, all `isTextOnly: true` (these are CCAT verbal/numerical text questions), most with `answerVisible: true`. Items where `explanationVisible: false` should additionally print the generated explanation from branch 4b. No POSTs, no log files written (dry-run).
...
These were open questions in an earlier draft of this plan; the user resolved both before implementation.

1. **Visible-answer + missing-explanation items get a generated explanation, not an empty one.** Implemented as branch 4b: a single Sonnet text-only call with the screenshot's answer treated as ground truth, returning a 1–3 sentence prose explanation. No verify pass — there is nothing to disagree with when the answer is already known. Logged with `source: "ocr-visible-explanation-generated"` so the provenance is auditable from `imported.jsonl`. Rationale: the user reviews wrong answers post-session, and an item without an explanation is a worse review experience than the marginal LLM cost of one extra Sonnet call.

2. **Difficulty estimation accepts Sonnet's call; no calibration against `latencyThresholdMs`.** The candidate-promotion workflow (Phase 6) rebalances based on empirical attempt data, which is the only honest difficulty signal. To make Sonnet's estimates more consistent, the extract prompt now ships with concrete feature-anchored definitions for easy / medium / hard / brutal (vocabulary level, arithmetic depth, pattern complexity) instead of the latency-window descriptions used elsewhere in the system.
