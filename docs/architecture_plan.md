...
**2. Item bank.** PostgreSQL via Drizzle. `items` carries a discriminated `body` JSON (today: a single `{ kind: 'text', text }` variant; the discriminated-union shape is preserved so future visual variants are additive), a uniform `options` array of `{ id, text }`, a difficulty tier, and a status (`live`, `candidate`, `retired`). v1 has no image storage — items are fully text-based — so there is no S3, no signed-URL minting, and no image route handler. The `pgvector` extension on `items.embedding` powers the validator's uniqueness check.
...
4. **Mastery Map.** Server-rendered. 11-icon grid (verbal: BookOpen × 5; numerical: Calculator × 6; fill state per mastery level), single-line near-goal text, one primary CTA naming the recommended next session, 30-day rolling triage adherence in low-contrast periphery.
...
Local development uses a `pgvector/pgvector:pg16` Docker container. Production runs on Vercel with AWS RDS via OIDC federation. LLM access is server-side only — no client SDKs, no streaming. Secrets (`AUTH_GOOGLE_*`, `AUTH_SECRET`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `CRON_SECRET`) flow through T3 Env. Production lands as a Vercel **preview deployment first**; promotion to production is a one-click move at launch. v1 has no S3 — there are no images to store.

## Major risks and mitigations

**v1 cannot prepare users for ~30% of the real test.** Abstract reasoning and attention-to-detail are deferred. Users prepping for the real CCAT need to know this; the post-diagnostic review explicitly names the uncovered sections, and the 11-icon Mastery Map (vs. the full 18) makes the scope visible. Deliberate scoping, not a system failure.
...
**Latency drift undermines the mastery model.** Server-rendered first item so first paint is the question becoming visible, latency anchored on the `<ItemSlot>` mount effect, `requestAnimationFrame` for timer ticks. (When future visual variants land, latency will need to anchor on text paint specifically rather than first paint.)
...
**Phase 2 — Real-item path (week 1, days 3–5).** Admin ingest form + tagger LLM call. Hand-seed ~150 real items distributed across the 11 v1 sub-types. Embedding-backfill workflow. Validates the (single-variant) body discriminator end-to-end.
